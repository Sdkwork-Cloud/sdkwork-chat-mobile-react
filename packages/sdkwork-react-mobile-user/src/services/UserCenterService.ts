import { APP_SDK_AUTH_TOKEN_STORAGE_KEY, getAppSdkCoreClientWithSession } from '@sdkwork/react-mobile-core';
import type {
...
function mapProfile(profile: UserProfileVO): UserCenterProfile {
  const userId = firstNonEmpty(profile?.email, profile?.phone, profile?.nickname, 'current-user');
  const nickname = firstNonEmpty(profile?.nickname, profile?.email, profile?.phone, 'User');
  return {
    userId,
    nickname,
...
class UserCenterServiceImpl implements IUserCenterService {
  async getUserProfile(): Promise<UserCenterProfile | null> {
    const client = await getAppSdkCoreClientWithSession({
  ...
  async bindEmail(email: string, verifyCode?: string): Promise<UserCenterProfile> {
    ...
    const data = unwrapResult<UserProfileVO>(
      response as PlusApiResultUserProfileVO,
